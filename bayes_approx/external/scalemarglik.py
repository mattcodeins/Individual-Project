...
def marglik_optimization(model,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for X, y in train_loader:
            ...
            f = model(X)
            loss = criterion(f, y) + (0.5 * (delta * theta)
                                      @ theta) / N / crit_factor
            loss.backward()
            optimizer.step()
            ...
            if likelihood == 'regression':
                ...
                epoch_perf += torch.sum(torch.argmax(f.detach(),
                                        dim=-1) == y).item() / N
            scheduler.step()
        ...
        if valid_loader is not None:
            with torch.no_grad():
                valid_perf = valid_performance(
                    model, valid_loader, likelihood, device)
        ...
        sigma_noise = 1 if likelihood == 'classification' else torch.exp(
            log_sigma_noise)
        ...
        if margliks[-1] < best_marglik:
            ...
            best_sigma = 1 if likelihood == 'classification' else deepcopy(
                sigma_noise.detach())
    ...
    logging.info(
        'MARGLIK: finished training. Recover best model and fit Lapras.')
