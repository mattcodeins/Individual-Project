...
class CollapsedMeanLoss(_Loss):
    ...
    """
    def __init__(self):
        super(CollapsedMeanLoss, self).__init__()
    ...
    def forward(self, model):
        ...
        Arguments:
            model (nn.Module): a model to be calculated for KL-divergence.
        """
        return BF.cm_loss(model)


class CollapsedMeanVarLoss(_Loss):
    """
    Loss for calculating KL divergence of baysian neural network model.
    ...
    """
    def __init__(self):
        super(CollapsedMeanVarLoss, self).__init__()
